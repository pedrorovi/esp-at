...
#define INIT_SPI_ETH_MODULE_CONFIG(eth_module_config, num)                     \
  do {                                                                         \
    ...
    eth_module_config[num].phy_reset_gpio =                                    \
        CONFIG_ETH_SPI_PHY_RST##num##_GPIO;                                    \
...
bool esp_at_get_eth_default_config() {
  ...
  //   ETH_ESP32_EMAC_DEFAULT_CONFIG(); esp32_emac_config.smi_mdc_gpio_num =
  //   CONFIG_AT_ETH_MDC_GPIO; esp32_emac_config.smi_mdio_gpio_num =
  //   CONFIG_AT_ETH_MDIO_GPIO; esp_eth_mac_t *mac =
  ...
  for (int i = 0; i < CONFIG_SPI_ETHERNETS_NUM; i++) {
    ...
    eth_ksz8851snl_config_t ksz8851snl_config =
        ETH_KSZ8851SNL_DEFAULT_CONFIG(CONFIG_ETH_SPI_HOST, &spi_devcfg);
    ksz8851snl_config.int_gpio_num = spi_eth_module_config[i].int_gpio;
    mac_spi[i] =
        esp_eth_mac_new_ksz8851snl(&ksz8851snl_config, &mac_config_spi);
    phy_spi[i] = esp_eth_phy_new_ksz8851snl(&phy_config_spi);
#elif CONFIG_ETH_SPI_ETHERNET_DM9051
    eth_dm9051_config_t dm9051_config =
        ETH_DM9051_DEFAULT_CONFIG(CONFIG_ETH_SPI_HOST, &spi_devcfg);
    ...
    /* The SPI Ethernet module might not have a burned factory MAC address, we
    ...
    */
    ESP_ERROR_CHECK(
        esp_eth_ioctl(eth_handle_spi[i], ETH_CMD_S_MAC_ADDR,
                      (uint8_t[]){0x02, 0x00, 0x00, 0x12, 0x34, 0x56 + i}));
...
// #endif // CONFIG_ETH_USE_ESP32_EMAC
  ...
  ESP_LOGI(TAG, "SPI Driver installed correctly ");
