...
static void http_native_request(void)
{
    ...
    if (err != ESP_OK) {
        ...
        // snprintf(buffer, 200, "Failed to open HTTP connection: %s", esp_err_to_name(err));
    } else {
        content_length = esp_http_client_fetch_headers(client);
        if (content_length < 0) {
            ESP_LOGE(TAG, "HTTP client fetch headers failed");
            
            // snprintf(buffer_1, 200, "HTTP client fetch headers failed");
        } else {
            ...
            if (data_read >= 0) {
                ...
                snprintf(buffer_2, 200, "HTTP GET Status = %d, content_length = %d",
                esp_http_client_get_status_code(client),
                esp_http_client_get_content_length(client));
                
                ESP_LOG_BUFFER_HEX(TAG, output_buffer, strlen(output_buffer));
                
                // snprintf(buffer_3, strlen(output_buffer), output_buffer);

            } else {
                ESP_LOGE(TAG, "Failed to read response");
                
                // snprintf(buffer_4, 200, "Failed to read response");
            }
        }
    }
    esp_http_client_close(client);
